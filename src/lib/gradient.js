...
class App {
  ...
  setColorScheme() {
    const colors = this.colorSchemes[5];
...
function initCursor() {
  const getCursor = () => document.getElementById("customCursor");
  let mouseX = 0;
  let mouseY = 0;
...
export class GradientApp {
  start() {
    console.log("[GradientApp] start() called");

    if (this.app) {
      console.log("[GradientApp] already exists");
      return;
    }

    console.log("[GradientApp] creating App()");
    this.app = new App();

    console.log("[GradientApp] App created successfully");

    initCursor();
    console.log("[GradientApp] cursor init done");
  }
}
